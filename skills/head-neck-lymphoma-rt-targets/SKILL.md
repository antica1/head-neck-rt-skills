...
metadata:
  hermes:
    ...
    triggers_on: [淋巴瘤, 霍奇金, 非霍奇金, ISRT, INRT, 受累部位放疗, 受累淋巴结放疗, ENKTL, 鼻型NK/T, 眼眶MALT, lymphoma, hodgkin, non-hodgkin, 靶区教学, 教学版]
...
> **模型注意力焦点——以下规则在所有头颈部淋巴瘤放疗中自动适用，长报告中的靶区描述必须与之完全一致。**

| # | 铁律 | 触发条件 |
|:--:|------|---------|
| 1 | **ISRT 是首选框架**：CTV = 化疗前 PET/CT 上的受累淋巴结站/结外区，经解剖边界修剪——不是照整条淋巴链、不做下级引流预防、不照对侧 | 所有头颈淋巴瘤 RT |
| 2 | **INRT 的前提是化疗前高质量 PET/CT 必须可用**；靶区为单个阳性淋巴结外扩 + 解剖修剪，QA 要求极严，无使用经验时慎用 | 拟行 INRT |
| 3 | **先辨 HL 还是 NHL**：HL 连续性蔓延、20-30 Gy、包整个受累解剖室；NHL 跳跃性蔓延、24-36 Gy、包受累淋巴结本身 + 同站相邻小淋巴结 | 所有病例的第一决策 |
| 4 | **剂量按亚型与化疗反应定**：DLBCL 化疗后 CR 巩固 30 Gy/15fx（PR 30-36 Gy/15-18fx、单纯 RT 36-40 Gy）；FL 1-2 级 24 Gy/12fx；FL 3 级 30 Gy/15fx；MALT 24-30 Gy/12-15fx；MCL 30-36 Gy | 亚型与化疗反应已定 |
| 5 | **DLBCL 放疗决策走风险分层**（2024 JNCC 荟萃，李晔雄团队）：PFS ≤40% 高危获益最大（OS 绝对获益约 21%，HR 0.48）；40-80% 中危获益递增 | DLBCL 化疗后巩固决策 |
| 6 | **眼眶 MALT 可用超低剂量 4 Gy/2fx**（48h 间隔，CR 92%，"Boom-Boom RT"）；残留追加至 24 Gy；双侧可同时照射不增加毒性 | 眼附属器 MALT、体积小/症状轻 |
| 7 | **4 Gy 超低剂量不适用于 DLBCL / MCL / HL**：眼眶 DLBCL 为 30-36 Gy/15-18fx（化疗后巩固），鼻腔鼻窦 DLBCL 扩展至眼眶时按眼眶原则但**不加超低剂量** | 眼眶非 MALT 亚型 |
| 8 | **ENKTL（鼻型）以放疗为核心**：局限期 CCRT 40 Gy（40-50 Gy/20-25 fx）+ 顺铂同步；中国"三明治"放化疗模式 | 鼻腔鼻窦 ENKTL |
| 9 | **眼眶 OAR 极度密集**：晶状体、角膜、视神经、泪腺须单独勾画并限量（在全身约束基础上叠加 §9.4） | 眼眶/眼附属器淋巴瘤 RT |
| 10 | **OAR 优先级随瘤种不同**：NHL 重点口腔黏膜（Dmean ≤30 Gy）、下颌骨（Dmax ≤50 Gy）、腮腺（同侧 ≤26 Gy、对侧 20-24 Gy）、脊髓 Dmax ≤40 Gy；HL 重点甲状腺（Dmean ≤20 Gy）、颈动脉（≤10 Gy）、乳腺、脊髓 Dmax ≤36 Gy | 制定计划约束时 |
| 11 | **颈部受累站按解剖室整体勾画**：ISRT 包整个受累解剖室（如整个右 Ib 区），Ib（颌下）/II（颈上深）/RPN（咽后）各有边界规则（§6.1-6.3），不按 mm 外放 | 颈部受累站勾画 |
| 12 | **化疗后 CR 与 PR 用不同剂量**：CR 用巩固剂量、PR 用根治剂量；未化疗者按单纯放疗剂量档 | 化疗后影像评估（PET-CR / PR） |
...
**本 skill 特有**：
- 注：淋巴瘤用 ISRT/INRT 框架（不照整条淋巴链、不做下级预防、不照对侧）；
