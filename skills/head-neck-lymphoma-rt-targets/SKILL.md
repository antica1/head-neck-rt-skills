...
> 治疗前写入住院病史「诊疗计划」。只列实际使用的 CTV 层级（ISRT：受累站/区），每层附理由；豁免区和加量区均说明原因；不含 OAR 限量（物理师负责）。

```text
═══════════════════════════════
  放疗靶区规划
═══════════════════════════════
诊断：______（亚型：______）  分期：______（Lugano / Ann Arbor）
分期判断：______（HL vs NHL：______；化疗前 PET 受累区：______）
化疗：______（方案×周期；末次化疗日期：______）
疗效：______（PET-CR / PR / SD）
放疗指征：______（巩固 / 残留根治 / 单纯放疗）

方案：□ 术后 PORT  □ 根治性 RT   ___ Gy / ___ fx（大野 ___Gy/___fx → 缩野加量 ___Gy/___fx）

CTV___：______（___ Gy — 理由：______）
加量：______  ___ Gy（理由：□残留 PR □大肿块 □局限期根治剂量）
豁免：______（理由：______）
...
剂量按亚型与化疗反应分档（DLBCL 巩固 30 Gy、FL 1-2 级 24 Gy、MALT 24-30 Gy、
眼眶 MALT 可 4 Gy/2fx）；4 Gy 超低剂量不适用于 DLBCL/MCL/HL。
```
