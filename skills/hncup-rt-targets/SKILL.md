...
metadata:
  hermes:
    ...
    triggers_on: [原发不明, 原发灶不明, 颈部转移癌, 不明原发灶, 颈部淋巴结转移, 颈部转移, CUP, HNCUP, cervical CUP, unknown primary, 颈部转移性癌, 颈部转移癌靶区, 颈部不明原发, EBV相关转移, HPV相关转移, 选择性黏膜照射, 选择性黏膜勾画, 逆流规则, 双侧颈部, 对侧颈预防, NPC筛查, 鼻咽镜阴性, PET阴性, 靶区教学, 教学版]
...
语气要求：像老师坐在旁边带着学生一步步看病例，循循善诱——先引导观察（"我们先看手术记录里写了什么、没写什么"），再顺势推导，不用"新手三个坑""解剖课""易错点"这类居高临下的标题式表述；比喻自然嵌入叙述，不堆砌标签。

1. **先看空间关系**（~200字）：用1个比喻带学生建立空间概念，比喻融入叙述而非贴标签
2. **分期里容易滑过去的地方**（~150字）：只讲本病例争议的 1-2 个判断，写清"为什么容易看错"
3. **靶区一层一层地推**（~600字核心）：4-6 层，每层顺着讲——先说照什么，再说为什么，最后说不照会留下什么风险
4. **剂量怎么给**（~150字）
5. **这个病例里容易踩的几处**（~200字）：以叙述口吻写出陷阱和依据，不编号训诫
6. **留几个问题想一想**（3题）：至少1题考豁免逻辑，答案倒排
...
> **模型注意力焦点——以下规则在所有原发不明颈部转移癌（HNCUP）靶区勾画中自动适用。**

| # | 铁律 | 触发条件 |
|:--:|------|---------|
| 1 | **真 CUP 门槛铁律**：五项检查全部穷尽仍无原发才成立——临床检查（鼻咽镜+喉镜+口腔触诊）、增强 MRI（首选）或增强 CT、颅底至大腿 FDG PET/CT、全景内镜+定向活检（双侧鼻咽活检、扁桃体切除或深部活检、舌根活检、可疑黏膜活检）、血浆 EBV DNA + 结节组织 HPV DNA/p16 | 任何拟诊颈部 CUP 的病例 |
| 2 | **疑似即非 CUP**：影像见任何黏膜不规则、不对称或可疑区——即使活检阴性——按该疑似原发处理，不按 CUP | 影像或内镜提示可疑黏膜灶 |
| 3 | **NPC 第一决策点**：Level IIb、V、RP 任一受累，或淋巴结高于 C1 平面，或血浆 EBV DNA 阳性，或结节组织 EBER 阳性——任一成立即按隐性鼻咽癌处理：全鼻咽靶区+双颈照射±化疗，除非大块/多发淋巴结不做颈清扫 | 每例 CUP 的 Step 1 分流 |
| 4 | **RP 淋巴结铁律**：咽后淋巴结（C1-C3 平面）受累为鼻咽癌近乎病理特征性；上颈 V 区（尤其 V 上部）受累亦高度提示鼻咽 | 由淋巴结位置反推原发 |
| 5 | **声门排除铁律**：真声带无固有淋巴管，T1-T2 声门癌不可能产生颈淋巴结转移——所有 CUP 病例黏膜 CTV 排除声门，除非有可见、活检证实的声门肿瘤 | 黏膜 CTV 勾画 |
| 6 | **选择性黏膜照射铁律**：只照射最可能起源的亚区，不做全咽轴+口腔泛黏膜照射——后者 3-4 级黏膜炎发生率 40-60%，而选择性照射黏膜控制率近 100% | Step 2 非 NPC 通路 |
| 7 | **四跨中线结构铁律**：黏膜 CTV 仅在四处跨中线——鼻咽顶后壁（至对侧咽隐窝）、软/硬腭（过中线 1-2 cm）、舌根-舌-口底（舌隔前部不完整）、杓会厌襞-会厌（疑声门上起源时） | 黏膜 CTV 对侧边界确定 |
| 8 | **黏膜可跨线、颈部不跨线**：黏膜 CTV 在上述四处跨中线，但颈部 CTV 保持同侧；对侧颈照射仅在病理证实对侧淋巴结阳性时进行 | 非 NPC 通路颈部 CTV |
| 9 | **同侧颈原则**：非 NPC 通路 + 对侧 cN0 → 仅照同侧颈；NPC 通路 → 双颈（标准 NPC 流程）；非 NPC 通路 + 对侧 cN(+) → 双颈 | 颈部照射范围决策 |
| 10 | **颈清后逆流铁律**：未手术时 VIII/IX/X 过站淋巴结几乎不受累、不做预防照射；颈清后且淋巴结负荷重（ECE+、≥3 枚 LN+、LN ≥3 cm）时下行淋巴梯度破坏 → 逆流/侧支充盈，应考虑将 VIII、IX 区纳入同侧 CTV | 颈清后靶区勾画 |
| 11 | **VI 区信标铁律**：孤立 VI 区淋巴结 → 前部口腔（口底前部、舌前尖、下前牙龈）；VI+IV 而无上颈淋巴结 → 声门下喉、颈段食管、甲状腺；无可见原发时黏膜 CTV 覆盖口底前部+声门下喉 | VI 区淋巴结阳性 |
| 12 | **剂量铁律**：隐性 NPC 通路 69.96 Gy/33 fx（SIB）；非 NPC 选择性黏膜 66 Gy/33 fx；同侧预防颈 54 Gy/30-33 fx；颈清床高危区 60-66 Gy/30-33 fx | 处方剂量设定 |
| 13 | **两步顺序铁律**：先 Step 1 除外鼻咽癌 → 再 Step 2 选择性同侧颈清扫（范围由淋巴结位置决定）+ 术后选择性黏膜放疗；不采用西方式颈清扫作为第一步 | 治疗顺序规划 |
